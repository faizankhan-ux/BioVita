...
import { User, Lock, Mail, ChevronRight, ArrowLeft, Loader2 } from 'lucide-react';
...
const PatientAuth = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      console.error("❌ [Auth] Google Error Code:", err.code);
      console.error("❌ [Auth] Google Error Message:", err.message);
      ...
      if (err.code === 'auth/operation-not-allowed') {
        ...
        userMessage = "The Sign-In popup was closed before completion.";
      }
          ...
          >
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <button 
                type="submit"
                disabled={loading}
                className="w-full py-4 rounded-2xl bg-white text-black font-black text-xs uppercase tracking-[0.2em] hover:bg-white/90 transition-all flex items-center justify-center gap-2 group disabled:opacity-50"
              >
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : (isLogin ? 'Sign In' : 'Get Started')}
              ...
              <button 
                ...
                disabled={loading}
                className="w-full py-4 rounded-2xl bg-white/5 border border-white/10 text-white font-bold text-xs uppercase tracking-[0.2em] hover:bg-white/10 transition-all flex items-center justify-center gap-3 group disabled:opacity-50"
              >
